...
class DeepSeekTranslator:
    ...
    def __init__(self, api_key=None):
        ...
        # Проверяем режим работы (локальный или облачный)
        self.use_local = env_settings.get('USE_LOCAL_MODEL', 'false').lower() == 'true'
        
        if self.use_local:
            # Настройка для локальной модели через Ollama
            self.base_url = env_settings.get('OLLAMA_BASE_URL', 'http://localhost:11434/v1')
            self.model = env_settings.get('OLLAMA_MODEL', 'deepseek-coder:6.7b-instruct')
            # Для Ollama не нужен API ключ, используем заглушку
            self.api_key = 'ollama'
            print(f"🏠 Используем локальную модель: {self.model}")
            print(f"   URL: {self.base_url}")
        else:
            # Настройка для облачного API
            self.api_key = api_key or env_settings.get('DEEPSEEK_API_KEY') or os.getenv("DEEPSEEK_API_KEY")
            self.base_url = "https://api.deepseek.com/v1"
            self.model = "deepseek-chat"
            if self.api_key:
                print(f"☁️  Используем облачный DeepSeek API")
            
        if self.api_key:
            # OpenAI-совместимый клиент работает и с Ollama
            self.client = OpenAI(
            ...
            self.client = None
            print("⚠️  API клиент не настроен. Проверьте настройки в .env")
    ...
    def _translate_with_deepseek(self, chapter_data, context):
        ...
        return {
            ...
            "translator": "Local Model" if self.use_local else "DeepSeek"
        }
    ...
    def _create_system_prompt(self, context):
        # Загружаем контекст из файла если есть
        from pathlib import Path
        import yaml
        
        context_file = Path('translation_context.yaml')
        if context_file.exists():
            try:
                with open(context_file, 'r', encoding='utf-8') as f:
                    translation_context = yaml.safe_load(f)
                    if 'system_prompt' in translation_context:
                        return translation_context['system_prompt']
            except:
                pass
