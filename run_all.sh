...
# Проверяем наличие book.pdf или book.txt
BOOK_FILE=""
if [ -f "book.txt" ]; then
    BOOK_FILE="book.txt"
    echo "📝 Найден текстовый файл: book.txt"
elif [ -f "book.pdf" ]; then
    BOOK_FILE="book.pdf"
    echo "📄 Найден PDF файл: book.pdf"
else
    echo "❌ Файл book.pdf или book.txt не найден!"
    echo "   Поместите PDF или TXT книгу в текущую директорию"
    exit 1
fi
...
# 2. Извлечение диаграмм (только для PDF)
if [[ "$BOOK_FILE" == *.pdf ]]; then
    echo "2️⃣ Извлечение диаграмм..."
    python3 02_extract_figures.py
    if [ $? -ne 0 ]; then
        echo "⚠️ Ошибка при извлечении диаграмм, продолжаем без них"
    else
        echo "✅ Диаграммы извлечены"
    fi
else
    echo "2️⃣ Пропускаем извлечение диаграмм (работаем с текстовым файлом)"
fi
echo ""
